...
> **Last reviewed:** 2026-08-26 · **Owner:** _{{team}}_ · **Issues:** _{{repo}}_
>
...
| CSS + JS | `https://cdn.ux4g.gov.in/UX4G@3.2.0/` | **3.2.0** | — |
| Web pkg | npm `ux4g-web-components` | **2.1.0** | 2026-09-05 |
...
> currently tell which npm version corresponds to CDN 3.1.0, or which Figma
...
| Ring (semantic) | Mode 1 | Focus-ring width, colour, offset, inset |
| Outline (semantic) | Mode 1 | Outline width, style, colour, offset |
| Divide (semantic) | Mode 1 | Between-child border width and colour |
| Effects (semantic) | Mode 1 | Blur, brightness, contrast, blend modes |
| Scroll (semantic) | Mode 1 | Scrollbar size, radius, track, thumb, snap |
| Line-clamp (semantic) | Mode 1 | Multi-line truncation line counts |
| Screen Mode | Mobile, Tablet, Desktop, Desktop XL | Breakpoint ranges + grid |

> **Added in the 3.1.0 line (npm 2.0.x):** dedicated primitive + semantic token
> files and matching utility modules for **rings, outlines, divides, effects,
> scroll, and line-clamp**. These expand the token surface beyond the original
> Theme collection; the class-level crosswalk lives in
> `public/mapping-classes/migration-class.md` (3,681 mapped classes across 38
...
lowercase, hyphenated. The stylesheet defines 1,100+ custom properties (grew from
the original 886 after the ring/outline/divide/effects/scroll/line-clamp token
additions in the 3.1.0 line), and the vast majority follow the convention. Dart
uses `Ux4g` PascalCase on every public symbol. Figma uses slash-nested Title Case
groups.
...
package is at 2.0.2 and has a small installed base.
...
<script src="https://cdn.ux4g.gov.in/UX4G@3.2.0/ux4g-custom.js"></script>
...
in `ux4g-web-components@2.0.2`. ❌ entries were confirmed absent by grepping the
...
9. **Version drift across artifacts** (§0, §11) — CDN 3.2.0, npm 2.1.0, pub 1.3.0, Figma 3.0, no published mapping.
